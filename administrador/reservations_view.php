...
if(isset($id)){
    ...
    $db->where('eid',$eid);
    $establiment=$db->getOne('establiments','persons');
...
?>
		...
		<script type="text/javascript">
            function updateState() {
				...
				//var restid = estate;
				var msg ="";
				var comments = <?php echo json_encode($ocomments); ?>;
				//restid = 3;
                var url ="update_reservation_state.php?restid=" + restid + "&resid=<?php echo $id; ?>&email=no";
		...
		<div id="main_container" class="main_container container_16 clearfix">
			...
			<div class="grid_16">
                <table class="static">
                	...
                	<tbody>
                        ...
                        <tr>
                            <?php 
                            $extra=0;
                            if($adults> $establiment['persons']) $extra=$adults -$establiment['persons']; ?>
                            <td><strong>Persones</strong></td>
                            <td><?php echo $persons; if($extra!=0)echo ' ('.$extra.' extra)';?></td>
                            <td></td>
                            <td></td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            ?>
								...
								?>
                                    ...
                                    <br /><br />
                                    <?php if($ocomments!="") { ?>
                                    <form name="dispon" id="dispon" action="update_reservation_state.php" method="get">
                                    ...
                                    <button class="button_colour orange round_all" style="clear:none" onClick="javascript:if (confirm('S\'enviarà un email al client indicant que no hi ha disponibilitat?')){window.location = 'update_reservation_state.php?restid=3&email=yes&resid=<?php echo $id;?>'};"><span>Sense Disponibilitat</span></button>
                                	</p>
                                    <?php } else { ?>
                                	<p>
                                    <button class="button_colour green round_all" style="clear:none" onClick="javascript:if (confirm('S\'enviarà un email al client demanant que faci el pagament?')){window.location = 'update_reservation_state.php?restid=2&resid=<?php echo $id;?>'};"><span>Disponible</span></button>
                                    <button class="button_colour orange round_all" style="clear:none" onClick="javascript:if (confirm('S\'enviarà un email al client indicant que no hi ha disponibilitat?')){window.location = 'update_reservation_state.php?restid=3&email=yes&resid=<?php echo $id;?>'};"><span>Sense Disponibilitat</span></button>
                                	</p>
								<?php
									}
								...
								?>
                                    <br /><br />
                                	<p>
                                    <button class="button_colour blue round_all" style="clear:none" onClick="javascript:if (confirm('S\'enviarà un email al client confirmant el pagament i la reserva?')){window.location = 'update_reservation_state.php?restid=4&resid=<?php echo $id;?>'};"><span>Pagament fet</span></button>
                                	</p>
                                <?php } ?>
                        ...
                        <tr>
                            <td style="font-size:14px; padding:10px;">
                                    ...
                                    <?php
                                        ...
                                        if ($num==0) {echo "<p style=\"margin-bottom:0px; font-size:12px;\">No hi ha historial d'Estats</p>";}
                                                            }
                                    ?>
                ...
                <table class="static">
                    <thead>
                    <tr>
                        <th colspan="8" align="left">Pagaments</th>
                    </tr>
                    ...
                    <?php
                        $db->orderBy('id','DESC');
                        $db->join("paymodules pm", "p.id_paymentway=pm.pid", "LEFT");
                        $db->where('resid',$id);
                        $rs_query=$db->get('payments p',null,'datetime, status, response_datetime, quantity, authorisation_code, response, error_code, id_transaction, pm.title_ca');
                        foreach($rs_query as $rs){
                    ?>
                        <tr>
                            <td><?php if($rs['status']=='0') echo '<span style="color:blue;">En proceso</span>'; elseif($rs['status']=='8') echo '<span style="color:red;">Cancelado</span>'; else echo '<span style="color:green;">Realizado</span>'; ?></td>
                            <td><?php echo $rs['title_ca']; ?></td>
                            <td><?php echo $rs['id_transaction']; ?></td>
                            <td><?php if(empty($rs['response_datetime'])) echo $rs['datetime']; else echo $rs['response_datetime']; ?></td>
                            <td><strong><?php echo round($rs['quantity'],2); ?> €</strong></td>
                            <td><?php echo $rs['authorisation_code']; ?></td>
                            <td><?php if($rs['response']=='0000') echo '<span style="color:green;">OK</span>'; else '<span style="color:red;">'.$rs['response'].'</span>'; ?></td>
                            <td><?php echo $rs['error_code']; ?></td>
                        </tr>
                    <?php 	}
                        if($db->count == 0) {
                    ?>
                            <tr>
                                <td colspan="8">Aún no hay pagos asociados a esta reserva</td>
                            </tr>
                    <?php 	}    ?>
                    </tbody>
                </table>
