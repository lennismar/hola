	...
	<script>
		$(document).ready(function() {
			$("#hero-home").backgroundCycle({
				imageUrls: [
				<?php
				if ($home['headimage1'] != "") echo "'image.php?width=1400&height=391&cropratio=1400:391&quality=70&image=/images/uploads/home/".$home['headimage1']."',"; else echo CDN_BASE."'assets/img/bg-home-1.jpg',";
				if ($home['headimage2'] != "") echo "'image.php?width=1400&height=391&cropratio=1400:391&quality=70&image=/images/uploads/home/".$home['headimage2']."',"; else echo CDN_BASE."'assets/img/bg-home-2.jpg',";
				if ($home['headimage3'] != "") echo "'image.php?width=1400&height=391&cropratio=1400:391&quality=70&image=/images/uploads/home/".$home['headimage3']."'"; else echo CDN_BASE."'assets/img/bg-home-3.jpg'";
				?>
					],
				fadeSpeed: 500,
				duration: 5000,
	...
	foreach($rs_query as $rs){
		if($cont_establiment<=3):
			if ($datein!="") {
				...
				$bestprice = round(GetBestPriceEstablimentPerNightPerPerson($rs['eid'],date("Y-m-d", strtotime("next saturday")),'','all',$_SESSION['persons']),2);
			}
...
//$query = mysql_query("SELECT idrecurso, image, idtipusrecurso, title_".$lng.", smalldescription_".$lng.", pvid, comid FROM recursos WHERE image<>'' ORDER BY RAND() LIMIT 4");
	$db->orderBy('rand()');
	$db->where('image','','<>');
	$rs_query=$db->get('recursos',4,"idrecurso, image, idtipusrecurso, title_".$lng.", smalldescription_".$lng.", pvid, comid, external_url");
...
<?php } ?>						  						  			  			  
			...
			<a class="btn--more push--top" href="<?php echo URL_HOME_RECURSOS_TURISTICOS; ?>"><?php echo DESCUBRE_MAS; ?></a>
...
<?php
		...
		foreach($resultado as $com){
			...
			echo '<a href="'.$lng.'/'.URL_SEARCH.'/'.urls_amigables($prov['title_url']) .'/'.$com['title_url'].'">'.$com['title'].'</a>,&nbsp;';
		}
